...
with tab_chefs:
    try:
        ...
        chefs = []
    if chefs:
        existing_specialties = sorted(list({c.specialty for c in chefs if c.specialty}))
    else:
    ...
    with col_form_c:
        ...
        c_name = st.text_input("Full Name", key="c_name_input")
        c_cpf = st.text_input("CPF", key="c_cpf_input")
        c_salary = st.number_input(
            "Salary ($)", min_value=0.0, step=100.0, key="c_salary_input"
        )
        spec_options = existing_specialties + ["➕ New Specialty..."]
        selected_spec_option = st.selectbox(
            "Specialty",
            options=spec_options,
            index=0 if existing_specialties else 0,
            key="c_spec_select",
        )
        if selected_spec_option == "➕ New Specialty...":
            final_specialty = st.text_input(
                "Enter Specialty", placeholder="e.g. Pastry", key="c_spec_manual"
            )
        else:
            final_specialty = selected_spec_option
        if st.button("Hire Chef", type="primary"):
